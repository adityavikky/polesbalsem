...
trait BpjsVClaimTraits
{
    public function requestGetBpjs($suburl)
    {
        try{
            ...
            $res = Http::withHeaders([
                'X-cons-id' => env('BPJS_VCLAIM_CONS_ID'),
                'X-timestamp' => $xTimestamp,
                'X-signature' => $this->createSign($xTimestamp, env('BPJS_VCLAIM_CONS_ID')),
    ...
    public function requestPostBpjs($suburl, $request)
    {
        try{
            ...
            $res = Http::accept('application/json')->withHeaders([
                ...
                'X-signature' => $this->createSign($xTimestamp, env('BPJS_VCLAIM_CONS_ID')),
                'user_key' => env('BPJS_VCLAIM_USER_KEY'),
            ])->withBody(json_encode($request), 'json')->post(env('BPJS_VCLAIM_BASE_URL').$suburl);
            return $this->responseDataBpjs($res->json(), $xTimestamp);
        } catch (\Exception $e) {
            $statusError['flag'] = 'RSUD Middleware Webservice';
            $statusError['result'] = 'Communication Errors With BPJS Kesehatan Webservice';
    ...
    public function requestPutBpjs($suburl, $request)
    {
        try{
            ...
            $res = Http::accept('application/json')->withHeaders([
                ...
                'X-signature' => $this->createSign($xTimestamp, env('BPJS_VCLAIM_CONS_ID')),
                'user_key' => env('BPJS_VCLAIM_USER_KEY'),
            ])->withBody(json_encode($request), 'json')->put(env('BPJS_VCLAIM_BASE_URL').$suburl);
            //return $res;
            return $this->responseDataBpjs($res->json(), $xTimestamp);
        } catch (\Exception $e) {
            $statusError['flag'] = 'RSUD Middleware Webservice';
    ...
    public function requestDeleteBpjs($suburl, $request)
    {
        try{
            ...
            )->withBody(json_encode($request), 'json')->delete(env('BPJS_VCLAIM_BASE_URL').$suburl, 'json');
            // dd($res);
            return $this->responseDataBpjs($res->json(), $xTimestamp);
        } catch (\Exception $e) {
            $statusError['flag'] = 'RSUD Middleware Webservice';
    ...
    {
        $data = $dataPar;
        $secretKey = env('BPJS_VCLAIM_CONS_PWD');
    ...
    {
        $consid = env('BPJS_VCLAIM_CONS_ID');
        $conspwd = env('BPJS_VCLAIM_CONS_PWD');
